        ...
        <div class="dropdown">
            <button class="d-flex align-items-center gap-2 btn btn-sm"
                ...
                <div class="d-none d-lg-block text-start" style="line-height: 1.2;">
                    <div style="font-size: 0.78rem; font-weight: 600; color: var(--text-primary, #E6EDF3); max-width: 120px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">
                        {{ Auth::user()->name }}
                    </div>
                    ...
                    @endphp
                    <span style="font-size: 0.6rem; font-weight: 700; color: {{ $badgeColor }}; letter-spacing: 0.5px; text-transform: uppercase;">
                        {{ $roleName }}
                    </span>
                    @endif
                </div>
                ...
                <li>
                    <div class="px-3 py-2 border-bottom border-light-dark">
                        <div class="d-flex align-items-center gap-2 mb-1">
                            <img src="https://ui-avatars.com/api/?name={{ urlencode(Auth::user()->name) }}&background=4B6EF5&color=fff&size=64&rounded=true"
                                 width="36" height="36" class="rounded-circle flex-shrink-0" alt="{{ Auth::user()->name }}">
                            <div class="min-w-0">
                                <div class="fw-semibold text-sm" style="color: var(--text-primary); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; max-width: 140px;">{{ Auth::user()->name }}</div>
                                <div class="text-muted text-xs" style="white-space: nowrap; overflow: hidden; text-overflow: ellipsis; max-width: 140px;">{{ Auth::user()->email }}</div>
                            </div>
                        </div>
                        @if($roleName)
