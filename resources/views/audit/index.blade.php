...
@section('content')
    <div class="d-flex align-items-center justify-content-between mb-4 flex-wrap gap-2">
        <div>
            ...
            <p class="text-muted text-sm m-0">Catatan riwayat aktivitas pengguna, perubahan data stok, status DO, dan akses
                login.</p>
        </div>
    </div>
        ...
        <form method="GET" action="{{ route('audit.index') }}" class="row g-2 align-items-end">
            ...
            <div class="col-md-3">
                ...
                <div class="input-group input-group-sm">
                    <span class="input-group-text bg-transparent border-end-0 border-light-dark"><i
                            class="bi bi-search text-muted"></i></span>
                    <input type="text" name="search"
                        class="form-control border-start-0 border-light-dark ps-0 bg-transparent text-white text-xs"
            ...
            <div class="col-md-2">
                ...
                <select name="user_id"
                    class="form-select form-select-sm bg-transparent text-white border-light-dark text-xs">
                    <option value="all" class="bg-dark text-white">Semua Pengguna</option>
                    @foreach ($users as $u)
                        <option value="{{ $u->id }}" {{ ($causer_id ?? 'all') == $u->id ? 'selected' : '' }}
            ...
            <div class="col-md-2">
                ...
                <select name="module"
                    ...
                    @foreach ($modules as $key => $label)
                        <option value="{{ $key }}" {{ ($module ?? 'all') == $key ? 'selected' : '' }}
                            class="bg-dark text-white">
                            {{ $label }}
                        </option>
                    @endforeach
            ...
            <div class="col-md-2 col-6">
                ...
                <input type="date" name="date_start"
                    class="form-control form-control-sm bg-transparent text-white border-light-dark text-xs"
                    value="{{ $date_start ?? '' }}">
            </div>
            ...
            <div class="col-md-1 d-flex gap-2">
                ...
                @if ($search || ($causer_id && $causer_id !== 'all') || ($module && $module !== 'all') || $date_start || $date_end)
                    <a href="{{ route('audit.index') }}" class="btn btn-outline-secondary btn-sm py-1.5"
                        title="Reset Filter">
                    ...
                    @forelse($activities as $activity)
                        @php
                            ...
                            $eventBadgeClass = match ($activity->event) {
                                ...
                                'stock_in',
                                'activate'
                                    => 'bg-success text-success bg-opacity-10 border border-success border-opacity-20',
                                'logout'
                                    => 'bg-secondary text-secondary bg-opacity-10 border border-secondary border-opacity-20',
                                'password_reset',
                                'password_change',
                                'adjustment'
                                    => 'bg-warning text-warning bg-opacity-10 border border-warning border-opacity-20',
                                'updated',
                                'change_role'
                                    => 'bg-primary text-primary bg-opacity-10 border border-primary border-opacity-20',
                                'deleted',
                                'stock_out',
                                'deactivate'
                                    => 'bg-danger text-danger bg-opacity-10 border border-danger border-opacity-20',
                            ...
                            $userName = $causer
                                ? $causer->name
                                : ($activity->causer_id
                                    ? "Deleted User (ID: {$activity->causer_id})"
                                    : 'System');
                            $userRole = $causer
                                ? $causer->roles->first()?->name ?? 'No Role'
                                : ($activity->causer_id
                                    ? '-'
                                    : 'System');
                            ...
                            $roleBadgeClass = match ($userRole) {
                                ...
                                'Kepala Produksi' => 'bg-primary text-primary',
                                'Mandor' => 'bg-success text-success',
                                'Kepala Lapangan' => 'bg-danger text-danger',
                        ...
                        <tr>
                            <td class="text-muted">
                                {{ $loop->iteration + ($activities->currentPage() - 1) * $activities->perPage() }}</td>
                            ...
                            <td>
                                @if ($userRole === 'System')
                                    <span
                                        class="badge bg-secondary bg-opacity-15 border border-secondary border-opacity-25 fw-semibold px-2">System</span>
                                @elseif($userRole === '-')
                                    <span class="text-muted">-</span>
                                @else
                                    <span
                                        class="badge {{ $roleBadgeClass }} bg-opacity-15 border border-opacity-25 fw-semibold px-2">
                                        {{ $userRole }}
                                    </span>
                                @endif
                            </td>
                            ...
                            <td>
                                @if ($activity->log_name === 'inventory')
                                    <span class="badge bg-opacity-10 px-2 py-1 text-xs"
                ...
                <div class="modal-body">
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <pre class="p-3 rounded text-xs overflow-auto border border-light-dark" id="detail-old"
                                style="max-height: 350px; color: #f87171; background: #0f172a; border-color: #334155 !important;"></pre>
                        </div>
                        <div class="col-md-6 mb-3">
                            <label class="form-label text-xs text-muted mb-1">Nilai Baru (Sesudah)</label>
                            <pre class="p-3 rounded text-xs overflow-auto border border-light-dark" id="detail-new"
                                style="max-height: 350px; color: #4ade80; background: #0f172a; border-color: #334155 !important;"></pre>
                        </div>
                    </div>
                    <div class="mt-2">
                        <label class="form-label text-xs text-muted mb-1">Seluruh Metadata / Parameter</label>
                        <pre class="p-3 rounded text-xs overflow-auto border border-light-dark" id="detail-full"
                            style="max-height: 250px; color: #e2e8f0; background: #0f172a; border-color: #334155 !important;"></pre>
                    </div>
                </div>
