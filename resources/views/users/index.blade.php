            ...
            <table class="table align-middle text-sm mb-0">
                ...
                <tbody>
                    @forelse($users as $user)
                        <tr>
                            <td class="text-muted">{{ $loop->iteration + ($users->currentPage() - 1) * $users->perPage() }}
                            </td>
                            <td>
                                <div class="d-flex align-items-center gap-3">
                                    ...
                                    <div>
                                        <div class="fw-semibold" style="color: var(--text-primary);">{{ $user->name }}
                                        </div>
                                        @if ($user->id === auth()->id())
                            ...
                            <td>
                                ...
                                @if ($roleName)
                                    ...
                                    @endphp
                                    <span
                                        class="badge {{ $badgeClass }} bg-opacity-15 border border-opacity-25 fw-semibold px-2">
                                        {{ $roleName }}
                                    </span>
                                @else
                            ...
                            <td>
                                @if ($user->is_active ?? true)
                                    <span
                                        class="badge bg-success text-success bg-opacity-15 border border-success border-opacity-25 fw-semibold px-2">
                                        <i class="bi bi-check-circle me-1"></i>Aktif
                                    </span>
                                @else
                                    <span
                                        class="badge bg-danger text-danger bg-opacity-15 border border-danger border-opacity-25 fw-semibold px-2">
                            ...
                            <td class="text-center">
                                <div class="d-flex justify-content-center gap-2">
                                    <button class="btn btn-sm btn-outline-primary" data-bs-toggle="modal"
                                        ...
                                        data-user-name="{{ $user->name }}" data-user-role="{{ $roleName ?? '' }}"
                                    ...
                                    @if ($user->id !== auth()->id())
                                        <form method="POST" action="{{ route('users.toggleStatus', $user) }}"
                                            class="d-inline"
                                            onsubmit="return confirm('Apakah Anda yakin ingin mengubah status aktif user ini?')">
                                            @csrf
                                            @method('PUT')
                                            <button type="submit"
                                                class="btn btn-sm {{ $user->is_active ?? true ? 'btn-outline-danger' : 'btn-outline-success' }}"
                                                title="{{ $user->is_active ?? true ? 'Nonaktifkan User' : 'Aktifkan User' }}">
                                                <i
                                                    class="bi {{ $user->is_active ?? true ? 'bi-person-x' : 'bi-person-check' }}"></i>
                                            </button>
                                        </form>
                                    @else
                            ...
                            @foreach ($roles as $role)
                                @php
                                    $iconMap = [
                                        ...
                                        'Kepala Produksi' => 'bi-diagram-3-fill text-primary',
                                        'Mandor' => 'bi-house-gear-fill text-success',
                                        'Kepala Lapangan' => 'bi-signpost-split-fill text-danger',
                                ...
                                <label class="d-flex align-items-center gap-3 p-3 rounded cursor-pointer role-option"
                                    ...
                                    <div>
                                        ...
                                        <div class="text-muted text-xs">{{ $role->permissions->count() }} permissions
                                        </div>
                                    </div>
                                </label>
                            @endforeach
